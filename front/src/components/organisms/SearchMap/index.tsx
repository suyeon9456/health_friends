...
const SearchMap = ({ foldedFriends, setFoldedFriends }: {
  ...
  useEffect(() => {
    ...
    map?.current?.addControl(mapTypeControl, (window as any).kakao.maps.ControlPosition.TOPRIGHT);
    // 지도 확대 축소를 제어할 수 있는 줌 컨트롤을 생성
    const zoomControl = new (window as any).kakao.maps.ZoomControl();
    map?.current?.addControl(zoomControl, (window as any).kakao.maps.ControlPosition.RIGHT);
