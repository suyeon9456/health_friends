...
function loadGymsAPI(data: LoadGymProps): Promise<AxiosResponse<Gyms>> {
  return axios.get(
    `/gyms?lastId=${data?.lastId ?? 0}&searchWord=${
      data?.searchWord || ''
    }&swLon=${data?.swLon}&swLat=${data?.swLat}&neLon=${data?.neLon}&neLat=${
      data?.neLat
    }`
  );
}
...
function* loadFriends(
  action: PayloadAction<{ gymId: number; lastId: number }>
) {
  try {
    console.log('testtt');
    const result: AxiosResponse<Gym> = yield call(
      loadFriendsAPI,
      action.payload
    );
